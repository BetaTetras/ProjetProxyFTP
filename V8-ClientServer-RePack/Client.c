...
int connecterSocket(const char* ip, int port, const char* libelle);
...
int connecterSocket(const char* ip, int port, const char* libelle){
    ...
    */
    ...
    // On affiche que le socket est crée 
...
int commandeProxy(int* sockCTRL, int* sockDATA, const char* ip){
    ...
    // Si l'utilisateur entre "bye" alors on renvois 1 => fin du client
    ...
    // Si c'est LIST, lire les données du canal DATA déja ouvert pentre le proxy et le client 
    if(isListCommand){
        ...
        while((n = read(*sockDATA, dataBuffer, MAXLENGHT-1)) > 0){
            dataBuffer[n] = '\0';
            printf("%s", dataBuffer); // Ecriture LIGNE PAR LIGNE
        }
        
        printf_RGB(0,255,255,"=============================\n");        // Délimitateur du LIST
        
        // IMPORTANT: Reconnecter AVANT de lire le message 226
        // (le proxy attend notre reconnexion avant de lire le 226)
        close(*sockDATA);
        // printf_RGB(255,255,0,"[INFO] Reconnexion au canal DATA...\n");
        *sockDATA = connecterSocket(ip, 40011, "DATA"); // Reconnection au canal DATA entre le clent et le proxy
