        ...
        */
        ...
        if(pid == 0){
            ...
            while(1){
                ...
                // ========== GESTION QUIT ==========
                if(strncmp(bufferClient, "QUIT", 4) == 0 || strncmp(bufferClient, "quit", 4) == 0){
                ...
                // Vérifier si c'est une commande LIST
                else if(strncmp(bufferClient, "LIST", 4) == 0){
                    printf_RGB(255,255,0,"\n-===-\n[INFO] Commande LIST détectée, passage en mode PASV\n");
                    ...
                    if(strncmp(bufferFTP, "227", 3) == 0){
                    ...
                    }else if(strncmp(bufferClient, "RETR", 4) == 0){
                        printf_RGB(255,255,0,"\n-===-\n[INFO] Commande RETR détectée, passage en mode PASV\n");
                        ...
                        if(strncmp(bufferFTP, "227", 3) == 0){
                            ...
                            // 5. Envoyer la commande RETR au serveur FTP
                            write(sockControleFTP, bufferClient, nClient);
                            
                            // 6. Lire la réponse "150 Opening BINARY mode..." ou erreur
                            nFTP = readLine(sockControleFTP, bufferFTP, MAXLENGHT-1);
                            bufferFTP[nFTP] = '\0';
                            write(cliCTRL, bufferFTP, nFTP);
                            printf_RGB(0,255,255,"[FTP] %s", bufferFTP);
                            
                            // Vérifier si la commande a réussi (code 150)
                            if(strncmp(bufferFTP, "150", 3) == 0){
                                ...
                                printf_RGB(255,255,0,"[DATA] Fichier transféré : %d bytes\n", totalBytes);
                                ...
                                // Erreur (550 Not a regular file, etc.)
                                printf_RGB(255,0,0,"[KO] Erreur lors du transfert\n");
                                close(sockDataFTP);
                                close(cliDATA);
